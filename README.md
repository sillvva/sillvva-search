...
| `key=10`<br>`key<10`<br>`key>=2024-01-01`    | When using numeric operators for numbers or dates, the token will become a "keyword_numeric" or "keyword_date" token with the operator provided. Dates must be formatted as `YYYY-MM-DD`, `YYYY-MM`, or `YYYY`.                |
| `key:10..20`<br>`key:2024-01-01..2024-12-31` | Range queries allow you to specify a range of values. For ranges, use `key:start..end`, where dates can be formatted as `YYYY-MM-DD`, `YYYY-MM`, or `YYYY`. The result will be two "keyword_numeric" or "keyword_date" tokens. |
