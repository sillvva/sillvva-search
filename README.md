...
| `key=10`<br>`key>=2024-01-01`<br>`key>=2024-01-01T00:00:00.000Z`                         | When using numeric operators for numbers or dates, the token will become a "keyword_numeric" or "keyword_date" token with the operator provided. Dates must be formatted as `YYYY-MM-DD`, `YYYY-MM`, `YYYY`, or full ISO format (`YYYY-MM-DDTHH:mm:ss.sssZ` or `YYYY-MM-DDTHH:mm:ss.sss(+/-)HH:mm`). Seconds, milliseconds, and time zone are optional in the ISO format.            |
| `key:10..20`<br>`key:2024-01-01..2024-12-31`<br>`key:2024-01-01 00:00..2024-01-15 12:00` | Range queries allow you to specify a range of values. For ranges, use `key:start..end`. The result will be two "keyword_numeric" or "keyword_date" tokens. Dates must be formatted as `YYYY-MM-DD`, `YYYY-MM`, `YYYY`, or full ISO format (`YYYY-MM-DDTHH:mm:ss.sssZ` or `YYYY-MM-DDTHH:mm:ss.sss(+/-)HH:mm`)). Seconds, milliseconds, and time zone are optional in the ISO format. |
